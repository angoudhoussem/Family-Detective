...
public class Inscription extends Activity {
	EditText email, nom, prenom, password, telephone, adresse;
	String Nom, Prenom, Email, Password, Telephone;
	Button connexion;
	ImageView img;
	String result = null;
	InputStream is;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		telephone = (EditText) findViewById(R.id.id_telephone);
		adresse = (EditText) findViewById(R.id.editText1);

		connexion = (Button) findViewById(R.id.id_inscription);
	...
	class Loade extends AsyncTask<String, String, String> {
		...
		protected String doInBackground(String... arg0) {
			...
			String telS = telephone.getText().toString();
			String adresseS = adresse.getText().toString();
			String req = nomS.replaceAll("\\s+","") + "/"
					+ prenomS.replaceAll("\\s+","") + "/"
					+ passwordS.replaceAll("\\s+","") + "/"
					+ emailS.replaceAll("\\s+","") + "/"
					+ adresseS.replaceAll("\\s+","") + "/"
					+ telS.replaceAll("\\s+","") +"/"
					+codeImage.toString();
			System.out.println("res"+req);

			HttpGet httpGet = new HttpGet(
					"http://10.0.2.2:8085/projet/webresources/entity.profil/ajout/"
							+ nomS.replaceAll("\\s+","") + "/"
							+ prenomS.replaceAll("\\s+","") + "/"
							+ telS.replaceAll("\\s+","") +"/"
							+ emailS.replaceAll("\\s+","") + "/"			
